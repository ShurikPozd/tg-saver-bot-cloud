...
def _settings_text(st: dict) -> str:
    ...
    lines = [
        ...
        "Распознавание фото — ИИ описывает картинку и по описанию подбирает категорию (арты, мемы...).",
        ...
        "Копия экспорта — раз в день присылать полный экспорт сюда (страховка от потери архива).",
    ]
    return "\n".join(lines)
