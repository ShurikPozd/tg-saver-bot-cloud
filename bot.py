...
def _safe_handler(fn):
    async def wrapper(event):
        try:
            ...
            try:
                await event.answer("⚠️ Внутренняя ошибка — подробности в логах", alert=True)
            except Exception:
                pass
...
async def cmd_start(event):
    await event.respond(
        ...
        "Кнопки под сообщением — мои категории, поиск и статистика.\n"
        "⚙️ Настройки — как пересылать сохранённое (отдельным сообщением или ответом).\n"
...
async def cmd_help(event):
    await event.respond(
        "🤖 Как пользоваться:\n\n"
        "• Пересылай мне посты, сообщения, альбомы — сохранятся в категории\n"
        "• 📂 Категории — все сохранённое по темам\n"
        "• ✏️ и ➕ — переименование и создание папок, категорий, постов\n"
        "• 🔍 Поиск — ищем по тексту\n"
        "• 📊 Статистика — сколько всего сохранено\n"
        "• ⚙️ Настройки — автозамок, распознавание фото и быстрые действия\n"
        "• 🔁 Дубли — найти и убрать повторяющиеся вложения в архиве\n\n"
...
def _settings_text(st: dict) -> str:
    ...
    ao_txt = "выкл" if ao_every in ("", "0") else f"каждые {ao_every} постов"
    ...
    lines = [
        ...
        "",
        f"• 🔒 Автозамок: {auto}",
        f"• 🖼️ Распознавание фото: {vision}",
        f"• ⚡️ Быстрые действия: {qact}",
        f"• 🧹 Чистка пустых категорий/папок: {cln}",
        f"• ⚡️ Авто-порядок: {ao_txt}",
        f"• 🧩 Подпапки: {sf}",
        f"• 🧩 Мин. постов в подпапке: {sfm}",
        f"• 🧩 Подпапки в авто-порядке: {sfa}",
        f"• 🔒 Гостевой режим: {gr}",
        f"• 🎙️ Распознавание голосовых: {av}",
        f"• 🔁 Проверка дублей: {dd}",
        f"• 📬 Дайджест за неделю: {wd}",
        f"• 🗄 Копия экспорта в TG: {bt}",
        "",
        "Автозамок ставится после ручных действий (перемещение, переименование, "
        "создание), чтобы автопорядок и деревья их не сдвигали.",
        "Распознавание фото — описание картинок через llava для категоризации «артов».",
        "Быстрые действия — кнопки «Переместить / Название / Теги» сразу после сохранения.",
        "Чистка пустых — авто-порядок удаляет категории и папки без единого поста. "
        "Выключи, если хочешь оставлять заготовки для будущих постов.",
        "Авто-порядок — сам объединяет дубли и раскладывает по папкам после N новых постов. "
        "«0» или «выкл» — совсем не запускать, только по кнопке.",
        ...
        "Голосовые — транскрибация голосовых/аудио через whisper, чтобы их можно было категоризировать.",
        "Дубли — проверка при сохранении, что такое вложение (по stable file_unique_id) уже сохранено.",
        "Дайджест — раз в неделю бот пришлёт статистику новых постов по категориям.",
        "Копия экспорта — раз в день присылать полный экспорт в этот чат (страховка от потери БД).",
    ]
    return "\n".join(lines)
